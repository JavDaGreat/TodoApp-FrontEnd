const login = async (username, password) => {
  const resp = await fetch("http://localhost:3500/login", {
    method: "POST",
...
const signUp = async (username, password, email) => {
  const resp = await fetch("http://localhost:3500/register", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
const AddTodo = async (id, accessToken, title, description) => {
  const resp = await fetch("http://localhost:3500/todo", {
    method: "POST",
    headers: {
...
const GetAllTodo = async (accessToken, id) => {
  const resp = await fetch(`http://localhost:3500/todo?id=${id}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json",
